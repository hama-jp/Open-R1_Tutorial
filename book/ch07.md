...
> R1-Zero の主要な訓練上の特徴を支えた RL アルゴリズムです。
...
GRPO の本質は **「価値関数を消した」** ではなく、
**「価値関数という推定器を、グループ統計という別の推定器に置き換えた」** ことです。
ベースラインを引く役目は両者に共通し、その担い手だけが違います。

![PPO と GRPO の比較（共通の骨格と、違うのはベースライン推定器だけ）](../images/ch07_ppo_vs_grpo.svg)

> 図7-1 で **赤い「◆」マーク** が付いた箱が両者の唯一の違いです。
> 左の「価値ネット V_ψ」が右では「グループ統計 μ, σ」に置き換わっただけで、
> 残り（プロンプト→方策→応答→報酬→アドバンテージ→clip+KL更新）はほぼ同型。

```mermaid
flowchart LR
    subgraph PPO
        direction TB
        P1[prompt x] --> R1[response y]
        R1 --> RM1[reward r]
        R1 --> V["value model V_ψ(s)"]
        RM1 --> ADV1["A = r − V_ψ(s)"]
        V --> ADV1
        ADV1 --> U1[policy update<br/>clip+KL]
    end
    subgraph GRPO
        direction TB
        P2[prompt x] --> G[G responses<br/>y^1 ... y^G]
        G --> RM2[rewards r^1 ... r^G]
        RM2 --> MS[group mean μ<br/>group std σ]
        MS --> ADV2["A^i = (r^i − μ) / σ"]
        ADV2 --> U2[policy update<br/>clip+KL]
    end
```

左右とも、最終的に「何らかの基準値を引いたアドバンテージで方策を更新する」 という骨格は共通です。
違うのは **ベースラインをどう見積もるか** だけです。
PPO は **状態依存の学習パラメータ** で、GRPO は **同一プロンプト内の統計量** で見積もります。
GRPO は「価値関数が無い代わりにサンプルを増やしてその場で統計を取る」という取引をしているわけです。
